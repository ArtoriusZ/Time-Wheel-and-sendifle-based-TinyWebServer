...
void http_conn::init(int sockfd, const sockaddr_in &addr, char *root, int TRIGMode,
                     int close_log, int zero_copy, string user, string passwd, string sqlname)
{
    m_sockfd = sockfd;
    m_address = addr;
...
//接收到完整的HTTP请求后，解析所请求的页面文件，并将该文件用mmap读取到内存中
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        if (*(p + 1) == '3')
        {
            ...
            if (users.find(name) == users.end())
            {
                //将用户加入users
                m_lock.lock();
                int res = mysql_query(mysql, sql_insert); //插入到数据库中
                users.insert(pair<string, string>(name, password)); //更新内存中的user表
                m_lock.unlock();

                //query成功返回注册页面，反之返回注册失败页面
                if (!res)
            ...
            //如果重名
                ...
                strcpy(m_url, "/registerError.html"); //重名则返回注册错误页面
        }
        //如果是登录，直接判断
...
{
    LOG_INFO("send file: %s",m_real_file);
    int temp = 0;
    //表示响应报文为空，一般不会出现这种情况
    if (bytes_to_send == 0)
    {
    ...
    {
        //cout<<"use sendfile:"<<endl;
        //先发送状态行，消息头和空行
        while (!send_head)
        {
            //将响应报文的状态行、消息头、空行发送给浏览器端
            temp = writev(m_sockfd, m_iv, m_iv_count);
            //error
            if (temp < 0)
            {
            ...
            //更新wei发送字节
            bytes_to_send -= temp;
            //第一个iovec头部信息的数据已发送完，退出循环去发送响应正文
            if (bytes_have_send >= m_iv[0].iov_len)
            {
                send_head=true;
                break;
        ...
        while(1)
        {
            off_t off=bytes_have_send-m_iv[0].iov_len;
            temp=sendfile(m_sockfd,m_file_fd,&off,bytes_to_send);
            //处理错误
            if(temp<0)
            {
                //判断缓冲区是否满了
